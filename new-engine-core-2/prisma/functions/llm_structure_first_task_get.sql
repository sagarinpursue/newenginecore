CREATE OR REPLACE FUNCTION public.llm_structure_first_task_get(structure_id uuid)
RETURNS TABLE (
    llm_structure_id uuid,
    name text,
    framework text,
    memory text,
    memory_size integer,
    memory_driver text,
    llm_task_id uuid,
    assistant_appendix text,
    preamble text,
    type text,
    llm_query_engine_id uuid,
    use_hybrid_search boolean,
    use_rag_api boolean,
    embedding_driver text,
    vector_store_driver text,
    namespace text,
    top_n integer,
    llm_prompt_driver_id uuid,
    model text,
    temperature numeric,
    top_p numeric,
    llm_ruleset_id uuid
...
SELECT
    ...
    s.framework,
    s.memory,
    coalesce(s.memory_size, 0) AS memory_size,
    s.memory_driver,
