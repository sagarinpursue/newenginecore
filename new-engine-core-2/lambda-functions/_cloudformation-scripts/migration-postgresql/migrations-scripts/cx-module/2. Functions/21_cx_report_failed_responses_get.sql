CREATE OR REPLACE FUNCTION cx_report_failed_responses_get(
    channel_filter TEXT,
    account_filter TEXT,
    search_filter TEXT,
...
AS $$
    ...
    WHERE
        ...
        (search_filter IS NULL OR search_filter = '' OR cma.question ILIKE '%' || search_filter || '%' OR cma.answer ILIKE '%' || search_filter || '%') AND
        cma.verdict = 'fail' AND cma.created_at BETWEEN p_start_date AND p_end_date
